...
abstract class BaseCobrosPeer {
	...
	/**
	 * Returns the number of rows matching criteria, joining the related RengCob table
	 *
	 * @param      Criteria $c
	 * @param      boolean $distinct Whether to select only distinct columns; deprecated: use Criteria->setDistinct() instead.
	 * @param      PropelPDO $con
	 ...
	 * @return     int Number of matching rows.
	 */
	public static function doCountJoinRengCob(Criteria $criteria, $distinct = false, PropelPDO $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		$criteria->addJoin(array(CobrosPeer::COB_NUM,), array(RengCobPeer::COB_NUM,), $join_behavior);
	...
	/**
	 * Selects a collection of Cobros objects pre-filled with their RengCob objects.
	 * @param      Criteria  $c
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	 ...
	 *		 rethrown wrapped into a PropelException.
	 */
	public static function doSelectJoinRengCob(Criteria $c, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
    ...
    foreach (sfMixer::getCallables('BaseCobrosPeer:doSelectJoin:doSelectJoin') as $callable)
    {
      call_user_func($callable, 'BaseCobrosPeer', $c, $con);
		...
		$c->addJoin(array(CobrosPeer::COB_NUM,), array(RengCobPeer::COB_NUM,), $join_behavior);
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$key2 = RengCobPeer::getPrimaryKeyHashFromRow($row, $startcol);
			if ($key2 !== null) {
				$obj2 = RengCobPeer::getInstanceFromPool($key2);
				if (!$obj2) {
					...
					RengCobPeer::addInstanceToPool($obj2, $key2);
				} // if obj2 already loaded

				// Add the $obj1 (Cobros) to $obj2 (RengCob)
				$obj2->setCobros($obj1);
    ...
    foreach (sfMixer::getCallables('BaseCobrosPeer:doSelectJoinAll:doSelectJoinAll') as $callable)
    {
      call_user_func($callable, 'BaseCobrosPeer', $c, $con);
		...
		RengCobPeer::addSelectColumns($c);
		$startcol3 = $startcol2 + (RengCobPeer::NUM_COLUMNS - RengCobPeer::NUM_LAZY_LOAD_COLUMNS);

		ClientesPeer::addSelectColumns($c);
		$startcol4 = $startcol3 + (ClientesPeer::NUM_COLUMNS - ClientesPeer::NUM_LAZY_LOAD_COLUMNS);
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			// Add objects for joined RengCob rows

			$key2 = RengCobPeer::getPrimaryKeyHashFromRow($row, $startcol2);
			if ($key2 !== null) {
				$obj2 = RengCobPeer::getInstanceFromPool($key2);
				if (!$obj2) {
				...
				// Add the $obj1 (Cobros) to the collection in $obj2 (RengCob)
				$obj1->setRengCob($obj2);
			} // if joined row not null

			// Add objects for joined Clientes rows

			$key3 = ClientesPeer::getPrimaryKeyHashFromRow($row, $startcol3);
			if ($key3 !== null) {
				$obj3 = ClientesPeer::getInstanceFromPool($key3);
				if (!$obj3) {
					...
					ClientesPeer::addInstanceToPool($obj3, $key3);
				} // if obj3 loaded

				// Add the $obj1 (Cobros) to the collection in $obj3 (Clientes)
				$obj3->addCobros($obj1);
			} // if joined row not null
			...
			if ($key4 !== null) {
				...
				// Add the $obj1 (Cobros) to the collection in $obj4 (Vendedor)
				$obj4->addCobros($obj1);
			} // if joined row not null
			...
			if ($key5 !== null) {
				...
				// Add the $obj1 (Cobros) to the collection in $obj5 (Moneda)
				$obj5->addCobros($obj1);
			} // if joined row not null
			...
			if ($key6 !== null) {
				...
				// Add the $obj1 (Cobros) to the collection in $obj6 (Almacen)
				$obj6->addCobros($obj1);
			} // if joined row not null
	 ...
	 * @return     int Number of matching rows.
	 */
	public static function doCountJoinAllExceptRengCob(Criteria $criteria, $distinct = false, PropelPDO $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		// we're going to modify criteria, so copy it first
		$criteria = clone $criteria;
	...
	/**
	 * Selects a collection of Cobros objects pre-filled with all related objects except RengCob.
	 *
	 * @param      Criteria  $c
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	 ...
	 *		 rethrown wrapped into a PropelException.
	 */
	public static function doSelectJoinAllExceptRengCob(Criteria $c, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
